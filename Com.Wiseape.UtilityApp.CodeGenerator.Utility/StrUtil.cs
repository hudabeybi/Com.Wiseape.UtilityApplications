...
namespace Com.Wiseape.UtilityApp.CodeGenerator.Helper
{
    ...
    {
        public static string LowFirst(string variable)
        {
            string var = variable.Substring(0, 1).ToLower() + variable.Substring(1, variable.Length - 1);
            return var;
        }
        public static string UpFirst(string variable)
        {
            string var = variable.Substring(0, 1).ToUpper()+ variable.Substring(1, variable.Length - 1);
            return var;
        }
        ...
        {
            string s = System.Text.RegularExpressions.Regex.Replace(input, "([A-Z])", separator + "$1", System.Text.RegularExpressions.RegexOptions.Compiled);
            if(separator != " ")
        ...
        public static string Pluralize(string text, int number = 2)
        {
            ...
            else
            {
                ...
                else if (text.EndsWith("ss", StringComparison.InvariantCultureIgnoreCase) || text.EndsWith("o", StringComparison.InvariantCultureIgnoreCase))
                {
                    return text + "es";
                }
